...
#include "ui_zBitcoinAdultcontroldialog.h"
...
std::list<std::string> ZBitcoinAdultControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBitcoinAdultControlDialog::listMints;

ZBitcoinAdultControlDialog::ZBitcoinAdultControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBitcoinAdultControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBitcoinAdultControlDialog::~ZBitcoinAdultControlDialog()
{
    delete ui;
}

void ZBitcoinAdultControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBitcoinAdultControlDialog::updateList()
{
...
void ZBitcoinAdultControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBitcoinAdultControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBitcoinAdult_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBitcoinAdultControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBitcoinAdultControlDialog::GetSelectedMints()
{
...
void ZBitcoinAdultControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

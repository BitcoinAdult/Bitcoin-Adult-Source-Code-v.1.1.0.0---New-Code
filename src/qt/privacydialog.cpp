                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zBTADpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBTADSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBTAD->setEnabled(false);
        ui->pushButtonMintzBTAD->setToolTip(tr("zBTAD is currently disabled due to maintenance."));

        ui->pushButtonSpendzBTAD->setEnabled(false);
        ui->pushButtonSpendzBTAD->setToolTip(tr("zBTAD is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBTADpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBTAD_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
...
void PrivacyDialog::on_pushButtonSpendzBTAD_clicked()
{
...
void PrivacyDialog::sendzBTAD()
{
    ...
    double dAmount = ui->zBTADpayAmount->text().toDouble();
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzBTADAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBTADSyncStatus->setVisible(fShow);
}
